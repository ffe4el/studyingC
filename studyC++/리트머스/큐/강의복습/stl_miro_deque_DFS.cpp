...
int main(){
    deque<Location2D> locdq;
    Location2D entry(1,0);
    locdq.push_front(entry); //덱에 입구 위치 삽입    

    while(locdq.empty()==false){
        Location2D here=locdq.front();  //덱의 front 상단 객체 복사
        locdq.pop_front(); //덱의 상단 객체 삭제
        ...
        if(map[r][c] =='x'){    // 출구를 만났으면 -> 탐색 성공
            ...
            if(isValidLoc(r-1,c)) locdq.push_front(Location2D(r-1,c));
            if(isValidLoc(r+1,c)) locdq.push_front(Location2D(r+1,c));
            if(isValidLoc(r,c-1)) locdq.push_front(Location2D(r,c-1));
            if(isValidLoc(r,c+1)) locdq.push_front(Location2D(r,c+1));
        }
    }
    printf("미로탐색실패\n");
