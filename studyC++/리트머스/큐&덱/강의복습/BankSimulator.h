...
class BankSimulator{
    ...
    int nCustomers;     //결과 : 전체 고객수
    int nServedCustomers;//결과 : 서비스 받은 고객 수
    CustomerQueue que;  //고객대기 큐

    //랜덤숫자를 생성하여 고객 도착 여부와 서비스 시간 자동 생성 코드
...
public:
    ...
    void run() {
        int clock=0;    //현재시각
        int serviceTime = -1; // 처리에 걸리는 잔여시간
        while(clock < nSimulation){
            ...
            if(serviceTime>0) serviceTime--;    //현재 고객 서비스중
            else{
