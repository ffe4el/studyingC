...
public :
    ...
    int path_sum(BinaryNode *node, int targetSum, int nowSum){
        if(node == NULL){
            return 0;
        }
        nowSum += node->getData();
        ...
        totalPath += path_sum(node->getLeft(), targetSum, nowSum);
        totalPath += path_sum(node->getRight(), targetSum, nowSum);

        return totalPath;
    }

    int countPath(int targetSum){
        int paths = countPath(root, targetSum);
        return paths;
    }

    int countPath(BinaryNode* root, int targetSum){
        if(root==NULL){
            return 0;
        }

        int pathFromRoot = path_sum(root, targetSum, 0);
        int pathOnLeft = countPath(root->getLeft(), targetSum);
        int pathOnRight = countPath(root->getRight(), targetSum);
